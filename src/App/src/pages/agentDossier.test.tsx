...
describe('the Agent dossier on the home surface', () => {
    ...
    it('lists the SOP tool and its gloss after the prompt but before configured facts', async () => {
        ...
        const tools = within(dossier).getByTestId('agent-dossier-tools');
        ...
        expect(prompt.compareDocumentPosition(tools) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
        expect(tools.compareDocumentPosition(configuration) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
    });
    ...
    it('states a knowledge base the pack switched off as no knowledge base at all', async () => {
        ...
        expect(within(dossier).queryByTestId('agent-dossier-tools')).not.toBeInTheDocument();
