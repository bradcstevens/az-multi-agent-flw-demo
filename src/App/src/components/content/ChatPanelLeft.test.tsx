...
describe('the chat-history panel drawer', () => {
    ...
    it('is a column the surface opens with, and the disclosure closes and returns (#168)', async () => {
        /*
          The default is the panel (ADR-048). A surface that opens with a hole
...
/*
  ...
  #130's chat-history drawer *was* an `OverlayDrawer`, and tabster made it a
  trapped modalizer. Under #168 it is a column and no longer traps anything,
  but the confirmation is still a modal Fluent `Dialog` with a modalizer of its
  own. These tests open it with `fireEvent`, which never moves focus, so that
  modalizer never becomes the active one and tabster's sweep marks the dialog's
  portal `aria-hidden="true"`. That sweep lands after these assertions on a
  developer's machine and before them on the runner, which is the whole of the
  difference between this file passing locally and failing in CI.
