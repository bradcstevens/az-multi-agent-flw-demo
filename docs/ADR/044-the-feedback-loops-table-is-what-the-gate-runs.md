# ADR-044: The Feedback loops table is what the gate runs, so nothing in it may observe a deployment
...
a *fresh worktree* (ADR-043), through a plain shell, unattended, with no `az login` behind it, on an
...
auto-resolution attempts on it — attempt 1 fixed a genuine Backend lint failure (ADR-043), the gate
...
"this loop cannot run here" from "this loop failed", which is the same conflation ADR-043 fixed for
...
- [ADR-043](./043-the-feedback-loops-virtualenv-is-shared-across-worktrees.md) — the fresh worktree
